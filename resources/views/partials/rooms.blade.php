...
function formatDate(date) {
    ...
    if (startDate && endDate) {
        ...
        if (!apendedRoom.length) {
            let sub_total = parseFloat(fare) * dayCount;
            let html = "";
            ...
            let subTotal = parseFloat(unitFare) * room.length;
    ...
    let totalDays = Math.ceil((endDate - startDate) / oneDay);

    // Adjust for partial days
    if (startDate.getHours() < 12) {
        totalDays += 1; // Include the day before the start date if starting before 12 PM
    }
    if (endDate.getHours() >= 12) {
        totalDays += 1; // Include the next day if ending after 12 PM
    }
