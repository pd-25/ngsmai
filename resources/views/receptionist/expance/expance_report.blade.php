...
@section('panel')
    <div class="row ">
        ...
        <div class="col-lg-12" style="display-flex">
            ...
            <form style="display: contents;" action="{{ route('receptionist.expance_report.filter_by_user') }}"
                method="post">
                @csrf
                <select style="margin-left: 20px; background-color: #ffff;" id="expense_category" name="id"
                    required="">
                    <option disabled selected>Filter Customer-Wise</option>
                    @forelse ($users as $user)
                        <option value="{{ $user?->id }}" <?php echo $user_id == $user?->id ? 'selected' : ''; ?>>{{ $user?->booking_number }} ||
                            <?php echo isset($user?->guest_details->name) ? $user?->guest_details->name : ''; ?> </option>
                    @empty
            ...
            <a href="{{ route('receptionist.expance_report.report') }}" class="btn btn-primary mx-5"
                                ...
                                @forelse($combinedData as $item)
                                    @if ($user_id == 0 && $item?->particulars != 'test')
                                        <tr>
                                            ...
                                            @endif
                                            <td style="text-align:left;" data-label="@lang('payment')">
                                                {{ $item?->guest_details?->payment_mode }}</td>
