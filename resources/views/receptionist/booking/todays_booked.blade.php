...
@section('panel')
    ...
    <div class="row mb-4">
        ...
        <div class="col-md-4 mb-2">
            <div class="card bg-info text-white">
                ...
                <div class="card-body text-center">
                    <p><b>{{ $totalRooms_1?->total_adult - $occupiedRooms_1 }}</b> out of
                        <b>{{ $totalRooms_1?->total_adult }}</b></p>
                </div>
            </div>
        </div>
        ...
        <div class="col-md-4 mb-2">
            <div class="card bg-info text-white">
                ...
                <div class="card-body text-center">
                    <p><b>{{ $totalRooms_2?->total_adult - $occupiedRooms_2 }}</b> out of
                        <b>{{ $totalRooms_2?->total_adult }}</b></p>
                </div>
            </div>
        </div>
        ...
        <div class="col-md-4 mb-2">
            <div class="card bg-info text-white">
                ...
                <div class="card-body text-center">
                    <p><b>{{ $totalRooms_3?->total_adult - $occupiedRooms_3 }}</b> out of
                        <b>{{ $totalRooms_3?->total_adult }}</b></p>
                </div>
            </div>
        </div>
    ...
    <div class="row mb-2">
        ...
        @forelse ($ExtraService as $Extra)
            <div class="col-md-4 mb-2 mt-2">
                <div class="card bg-info text-white">
                    <?php
                    $UsedExtraServices = DB::table('used_extra_services')
                        ->where('extra_service_id', $Extra->id)
                                ...
                                @forelse ($todaysCheckout as $item)
                                    <tr>
                                        ...
                                        <td style="text-align:left;" data-label="@lang('Booking Number')">
                                            <a href="{{ route('receptionist.booking.details', $item->booking_id) }}">
                                                {{ __($item->booking_number) }}
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                    stroke-width="1.5" stroke="currentColor" class="size-6" width="16"
                                                    height="16">
                            ...
                            <tbody>
                                ...
                                @forelse ($tomorrowsCheckout as $tomorrowsCheckou_t)
                                    <tr>
                                        ...
                                        <td style="text-align:left;" data-label="@lang('Booking Number')">
                                            <a
                                                href="{{ route('receptionist.booking.details', $tomorrowsCheckou_t->booking_id) }}">
                                                {{ __($tomorrowsCheckou_t->booking_number) }}
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                    stroke-width="1.5" stroke="currentColor" class="size-6" width="16"
                                                    height="16">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <div class="widget-two box--shadow2 b-radius--5 bg--white">
                    ...
                    <div class="widget-two__content d-flex align-items-end justify-content-between flex-wrap">
                        ...
                        <div>
                            @if ($room->booking)
                                @if ($room->booking->user_id)
                                    <h3>
                                        <a href="{{ route('receptionist.booking.detail', $room->booking->user_id) }}"
                                            class="f-size--18 text-center text--dark"><?php echo isset($room->booking->user->fullname) ? $room->booking->user->fullname : ''; ?></a>
                                    </h3>
                                @else
                                ...
                                <div class="d-flex flex-column fw-bold w-100">
                                    <p class="text--muted text--small">@lang('Booking No.'): <span
                                            class="fw-bold">{{ $room->booking->booking_number }}</span></p>
                                    <p class="text--muted text--small">@lang('Room Type'):
                                        {{ __($room->room->roomType->name) }}</p>
                                </div>
                            @else
                        ...
                        <div class="d-flex flex-wrap gap-2">
                            <a href="{{ route('receptionist.extra.service.add') }}?room={{ $room->room->room_number }}"
                                class="btn btn--xs btn-outline--dark" data-services="{{ $room->extraServices }}"> <i
                                    class="la la-plus"></i>@lang('Add Service')</a>

                            <button type="button" class="btn btn--xs btn-outline--info btn-view"
                                data-services="{{ $room->extraServices }}"> <i
                                    class="la la-eye"></i>@lang('View Services')</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <!-- Modal -->
    <div class="modal fade" id="extraServices" tabindex="-1" role="dialog" aria-labelledby="modelTitleId"
        aria-hidden="true">
