...
@php
$galleryData = DB::table('gallery_category')->get();
@endphp
@section('panel')
    <div class="row">
        <div class="col-lg-12">
                        ...
                        @endphp
                                @forelse($data as $item)
                                    <tr>
                                        ...
                                        <td data-label="@lang('category_id')">
                                            {{ __($item->gallery_category) }}
                                        </td>
                                        <td data-label="@lang('gallery_title')">
                                            <span class="fw-bold">{{ __($item->gallery_title) }}</span>
                                        </td>
                                       
                                        <td data-label="@lang('image')">
                                            <img style="height: 100px;width: 150px;" class="img-fluid" src="../assets/images/{{ __($item->image) }}" >
                                        ...
                                        <button type="button" class="btn btn-sm btn-outline--primary cuModalBtn"
                                                ...
                                                data-action="{{ route('admin.gallery.remove',$item->id) }}"
            ...
            <div class="modal-content">
                ...
                @csrf
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label> @lang('gallery_title')</label>
                            <input type="text" class="form-control" id="gallery_title" name="gallery_title" required>
                        </div>
                        <div class="form-group">
                            <label> @lang('Gallery Category')</label>
                            <!-- <input type="text" class="form-control" id="gallery_category" name="gallery_category" required> -->
                            <select name="gallery_category" class="form-control" required >
                                    <option value="">@lang('Select Category')</option>
                                    @foreach ($galleryData as $type)
                                        <option value="{{ $type->id }}">{{ $type->gallery_category }}</option>
                                    @endforeach
                                </select>
                        </div>
                        <div class="form-group">
